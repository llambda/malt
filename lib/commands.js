...
module.exports.ping = function (retval) {
	...
	log.info("I'm ping command and on the master now. My arguments are: " + JSON.stringify(args));

	var rrandom = function () {
		var args = Array.prototype.slice.call(arguments);
		log.info("I'm ping command and in the minion now. My arguments are: " + JSON.stringify(args));
	...
	return rr(rrandom, args).then(function (val) {
		...
		log.info("I'm ping command on the master, and the minion finished its work.");
...
module.exports.yumup = function () {
	return rr(function () {
		return child_process.execSync('yum update && yum upgrade').toString('utf8');
	});
}
